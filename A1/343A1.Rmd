...
$$
\begin{aligned}
MoreThan&One(from,\ to,\ time,\ status) :=\\
...
$$
\begin{aligned}
MoreThanOneNotAllRead := MoreThanOne\ -\ (MoreThanOne \bowtie HaveOnePersonAllRead)
\end{aligned}
$$
-Find the messages that are not last.
$$
\begin{aligned}
...
\end{aligned}
$$
-Find the last messages with unread status from senders who have sent messages with more than one status and they must be spammers since we have already pop off the safe people above.
$$
\begin{aligned}
...
$$
\begin{aligned}
Answer := WrongTimeDirectMsg\ \cup\ WrongTimeGroupMsg\ \cup\ MsgBetweenNoContact = \varnothing
\end{aligned}
$$
